...
/**
 * This class stands for undelete StructType
 * @subpackage Structs
 */
class Undelete extends AbstractStructBase
{
    /**
     * The ids
    ...
    public $ids;
    /**
     * Constructor method for undelete
     * @uses Undelete::setIds()
     * @param string[] $ids
     */
    ...
    public static function validateIdsForArrayConstraintsFromSetIds(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $undeleteIdsItem) {
            // validation for constraint: itemType
            if (!is_string($undeleteIdsItem)) {
                $invalidValues[] = is_object($undeleteIdsItem) ? get_class($undeleteIdsItem) : sprintf('%s(%s)', gettype($undeleteIdsItem), var_export($undeleteIdsItem, true));
            }
        }
    ...
    public static function validateIdsForLengthConstraintFromSetIds($values)
    {
        ...
        foreach ($values as $undeleteIdsItem) {
            ...
            if (mb_strlen($undeleteIdsItem) !== 18) {
                $invalidValues[] = var_export($undeleteIdsItem, true);
            }
        }
        if (!empty($invalidValues)) {
     ...
     * @param string[] $ids
     * @return \SFPartnerApi\v480\StructType\Undelete
     */
