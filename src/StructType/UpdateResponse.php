...
class UpdateResponse extends AbstractStructBase
{
    ...
    public static function validateResultForArrayConstraintsFromSetResult(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $updateResponseResultItem) {
            // validation for constraint: itemType
            if (!$updateResponseResultItem instanceof \SFPartnerApi\v480\StructType\SaveResult) {
                $invalidValues[] = is_object($updateResponseResultItem) ? get_class($updateResponseResultItem) : sprintf('%s(%s)', gettype($updateResponseResultItem), var_export($updateResponseResultItem, true));
            }
        }
     ...
     * @param \SFPartnerApi\v480\StructType\SaveResult[] $result
     * @return \SFPartnerApi\v480\StructType\UpdateResponse
     */
