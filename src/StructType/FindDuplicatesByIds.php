...
/**
 * This class stands for findDuplicatesByIds StructType
 * @subpackage Structs
 */
class FindDuplicatesByIds extends AbstractStructBase
{
    /**
     * The ids
    ...
    public $ids;
    /**
     * Constructor method for findDuplicatesByIds
     * @uses FindDuplicatesByIds::setIds()
     * @param string[] $ids
     */
    ...
    public static function validateIdsForArrayConstraintsFromSetIds(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $findDuplicatesByIdsIdsItem) {
            // validation for constraint: itemType
            if (!is_string($findDuplicatesByIdsIdsItem)) {
                $invalidValues[] = is_object($findDuplicatesByIdsIdsItem) ? get_class($findDuplicatesByIdsIdsItem) : sprintf('%s(%s)', gettype($findDuplicatesByIdsIdsItem), var_export($findDuplicatesByIdsIdsItem, true));
            }
        }
    ...
    public static function validateIdsForLengthConstraintFromSetIds($values)
    {
        ...
        foreach ($values as $findDuplicatesByIdsIdsItem) {
            ...
            if (mb_strlen($findDuplicatesByIdsIdsItem) !== 18) {
                $invalidValues[] = var_export($findDuplicatesByIdsIdsItem, true);
            }
        }
        if (!empty($invalidValues)) {
     ...
     * @param string[] $ids
     * @return \SFPartnerApi\v480\StructType\FindDuplicatesByIds
     */
