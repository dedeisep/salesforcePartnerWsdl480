...
class Create extends AbstractStructBase
{
    ...
    public static function validateSObjectsForArrayConstraintsFromSetSObjects(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $createSObjectsItem) {
            // validation for constraint: itemType
            if (!$createSObjectsItem instanceof \SFPartnerApi\v480\StructType\SObject) {
                $invalidValues[] = is_object($createSObjectsItem) ? get_class($createSObjectsItem) : sprintf('%s(%s)', gettype($createSObjectsItem), var_export($createSObjectsItem, true));
            }
        }
     ...
     * @param \SFPartnerApi\v480\StructType\SObject[] $sObjects
     * @return \SFPartnerApi\v480\StructType\Create
     */
