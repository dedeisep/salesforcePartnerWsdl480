...
class Update extends AbstractStructBase
{
    ...
    public static function validateSObjectsForArrayConstraintsFromSetSObjects(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $updateSObjectsItem) {
            // validation for constraint: itemType
            if (!$updateSObjectsItem instanceof \SFPartnerApi\v480\StructType\SObject) {
                $invalidValues[] = is_object($updateSObjectsItem) ? get_class($updateSObjectsItem) : sprintf('%s(%s)', gettype($updateSObjectsItem), var_export($updateSObjectsItem, true));
            }
        }
     ...
     * @param \SFPartnerApi\v480\StructType\SObject[] $sObjects
     * @return \SFPartnerApi\v480\StructType\Update
     */
